 ...
 * @author omarjcm
 */
public class LoadInstancesSShape extends Load {
    
    public LoadInstancesSShape() {
        super("instances_iu_sshape");
    }
        ...
        try {
            ...
            for (int i=0; i<Configuration.ORDERS_LENGTH.length; i++) {
                for (int j=0; j<Configuration.CAPACITY_PICK_DEVICE.length; j++) {
                    for (int k=0; k<40; k++) {
                        String directory = "SS O"+Configuration.ORDERS_LENGTH[i]+" C"+Configuration.CAPACITY_PICK_DEVICE[j]+"/";
                        String nameFile = initial + "s-"+Configuration.ORDERS_LENGTH[i]+"-"+Configuration.CAPACITY_PICK_DEVICE[j]+"-"+k+".txt";
                        count++;
                    ...
                    // esto debido al nombre de los archivos que se encuentran en las instancias utilizadas
...
//                    if (Configuration.ORDERS_LENGTH[i] == 60 && Configuration.CAPACITY_PICK_DEVICE[j] == 75)
  //                      initial = 41;
            ...
            Logger.getLogger(LoadInstancesSShape.class.getName()).log(Level.SEVERE, null, ex);
        }
        return instances;
    }
